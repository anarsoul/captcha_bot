...
async fn main() {
    ...
    let hashmap: Arc<DashMap<(i64, i64), i64>> = Arc::new(DashMap::new());
    ...
    let handler = Update::filter_message()
        .branch(
            Message::filter_new_chat_members()
                .endpoint(|msg: Message, chatmemb: Vec<User>, bot: AutoSend<Bot>, hashmap: Arc<DashMap<(i64, i64), i64>>| async move {
                    for user in chatmemb {
                        ...
                        let user_id = user.id;
                        hashmap.insert((chat_id, user_id), num1 + num2);
                        let hashmap = hashmap.clone();
                        tokio::spawn(async move {
                            tokio::time::sleep(Duration::from_secs(NSECS)).await;
                            log::info!("{} seconds elapsed!", NSECS);
                            if hashmap.contains_key(&(chat_id, user_id)) {
                                hashmap.remove(&(chat_id, user_id));
                                bot_clone.send_message(chat_id, "Время вышло! Вы будете удалены из чата!")
            ...
            .branch(
                ...
                .endpoint(
                    |msg: Message, bot: AutoSend<Bot>, hashmap: Arc<DashMap<(i64, i64), i64>>| async move {
